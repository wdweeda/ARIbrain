...
2. Answer queries given a TDP threshold, and create a TDPClusters object (type `?ARIbrain::TDPQuery` for more details).
    ...
    - `summary(TDPClusters, ..., rest = FALSE)`
    - `show(TDPClusters)`
    - `length(TDPClusters)`
    
    and output cluster image:
    
    - `writeCluster(TDPClusters, file, template, ...)`
...
Finding clusters with non-zero TDP threshold `threshold` indicates the presence of some signal in each cluster, however, `threshold` of 40%, 70% and 90% could be characterised as weak, moderate and strong spatial localisation, respectively.
...
# print cluster summary table (with information on the rest of the brain)
summary(tdpclusters, rest=TRUE)
# write cluster image
writeClusters(tdpclusters, file="aribrain.nii.gz", template=mask_name)
```
...
# (1) create an ARIBrainCluster object
aricluster <- ARIBrainCluster(Pmap, mask=mask)

# (2) answer queries: find all maximal clusters given a TDP threshold
tdpclusters <- TDPQuery(aricluster, threshold=0.7)
# print cluster summary table (without rest)
summary(tdpclusters)
# write cluster image
writeClusters(tdpclusters, file="aribrain.nii.gz", template=mask)
```
