...
#' @param map 3D map of logical values. \code{TRUE} if the voxel is to be clustered (e.g., it is supra-threshold).
#' @param max_dist maximum distance allowed to be in the same cluster. By default, \code{max_dist = sqrt(3)} comprises all the voxels up to the corners surrounding the target voxel. A value such as \code{max_dist = sqrt(2)} excludes the corners.
#' @return a 3D map (same size as \code{map}) with integer values identifying the cluster and 0 elsewhere.
#' @examples
#' \dontrun{
